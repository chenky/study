...
async function batchAdd(page, selector) {
    ...
    const helloBtns = await page.$$(selector)
    const len = helloBtns.length
    ...
    for (const btn of helloBtns) {
        const timeout = getRandomIntInclusive(rndDown, rndUp) * 1000
        console.log(`timeout is ${timeout}`)
        await sleep(timeout)
        _count++
        if (_count >= limitCount) return
        console.log(`btn is ${btn}`, ` _count is ${_count}`)
        await btn.click()
        if (_count === len + 1) {
            // 需要加载下一页了
            const body = await page.$('body')
